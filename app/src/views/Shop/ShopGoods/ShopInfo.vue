		...
		<div class="info-content">
			<section class="section">
				...
				<div class="delivery">
					...
					<div class="delivery-money">配送费￥{{info.float_delivery_fee}}</div>
				</div>
			</section>
			<div class="split"></div>
			<section class="section">
				<h3 class="section-title">活动与服务</h3>
				<div class="activity" v-if="info.activities">
					<div class="activity-item" :class="'activity-' + activity.icon_color" v-for="(activity, index) in info.activities" :key = index>
						<span class="content-tag">
							<span class="mini-tag">{{activity.icon_name}}</span>
						</span>
			...
			<section class="section">
				...
				<ul class="detail">
					...
					<li><span class="bold">营业时间</span> <span>{{info.opening_hours}}</span></li>
				</ul>
			</section>
		</div>
	</div>
...
export default {
	...
	mounted () {
		this.infoScroll = new BScroll(this.$refs.info, {
			click: true
		})
	...
	methods: {
		setUlWidth () {
			...
			this.$refs.ul.style.width = (liWidth + space) * size + 'px'
		}
	},
	watch: {
		info () {
